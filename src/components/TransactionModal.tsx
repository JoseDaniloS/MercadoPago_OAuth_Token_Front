    ...
    >
      ...
      >
        <header className="py-6 flex gap-3 items-center">
          ...
          <TextUppercase>ID: {transaction.id}</TextUppercase>
        </header>
            ...
            <div className="py-4 flex flex-col gap-4 w-full md:max-w-1/2">
              ...
              {transaction.payer?.identification?.number && (
                <TransactionModalField
                  ...
                  value={formatDocument(transaction.payer.identification.number)}
                />
              )}
              {transaction.payer?.address && (
                <TransactionModalField
                  label="endereço"
                  value={`${transaction.payer?.address?.city} - ${transaction.payer?.address.federal_unit}, ${transaction.payer?.address.neighborhood}, ${transaction.payer?.address?.street_name}, ${transaction.payer?.address?.street_number}, ${transaction.payer?.address?.zip_code}`}
                />
              )}
            </div>
            <div className="py-4 flex flex-col gap-4 w-full md:max-w-1/2">
              ...
              <div className="p-6 rounded-2xl flex flex-wrap items-center uppercase bg-midnight-light border border-text-gray text-green-400">
                ...
                {transaction.card?.last_four_digits && (
                  <p className="text-white">{transaction.card.last_four_digits}</p>
                )}
              </div>
